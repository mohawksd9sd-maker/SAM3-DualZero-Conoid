...
This repository contains the full SAM3 framework: a minimal geometric construction based on a 2D right conoid with 12 discrete binary-icosahedral (2I) bridges, a Dual-Zero hyperreal regulator, and an almost-commutative Lorentzian spectral triple. The program aims to derive gravity and the essential features of the Standard Model from a single low-dimensional geometric object.
...
- **Paper 17**: Complete rigorous foundations — analytic Dirac properties, full Lorentzian NCG axiom verification (including compact resolvent and bounded commutators), and essential uniqueness argument.
...
3. Reproduce all numerical results with one command:

```bash
docker build -t sam3 . && docker run sam3
├── papers/                          # All LaTeX source files (17–19, Flagship, etc.)
...
├── data/raw/                        # Raw eigenvalues, overlaps, Monte-Carlo data (Git LFS)
├── math/                            # Supplementary notebooks and proofs
...
19,Predictivity & Data Confrontation,May 2026,"Input/derived tables, BSM predictions"
02,Dual-Zero Hyperreal Regulator,Rewritten,Ultrapower + symmetric (\mathrm{Reg}_2)
05,Derivation of Gravity,Updated,( G_N = \frac{64\pi \ell_0^2}{45} )
...
Observable,SAM3 Prediction,Experimental / Notes
