...
of the SAM3-DualZero-Conoid Framework \\ (v4.26 — May 2026)}
...
\begin{abstract}
We present the quantitative predictions of the SAM3 framework for fermion masses, mixing angles, and other Standard Model observables. All results are derived from the geometric overlap integrals on the right conoid with Dual-Zero regularization and \(2I\) symmetry. The Dual-Zero regulator strength \(\omega_0\) is derived geometrically from the conoid structure. We provide full tables with numerical values, uncertainties, and direct comparison to experimental data.
\end{abstract}

\section{Introduction}
A geometric model of the Standard Model is only as strong as its ability to reproduce known observables. In this paper we report the numerical predictions obtained from the overlap integrals of Dirac eigenmodes on the right conoid with the \(2I\) projectors (as constructed in Papers 04 and 07). The regulator strength \(\omega_0\) is determined geometrically rather than tuned by hand.

\section{Numerical Setup}
\begin{itemize}
    \item Grid: \(160 \times 96\) warped finite-difference discretization of the conoid.
    \item Regulator: Dual-Zero strength \(\omega_0\) derived geometrically from the conoid as \(\omega_0 = (R_{\rm curvature}/D_{\rm bridge})^{4/13} \approx 0.927\).
    ...
    \item Scale setting: Top quark mass \(m_t = 173\) GeV used to fix the single fundamental dimensionful parameter \(\ell_0\).
\end{itemize}
...
\begin{itemize}
    ...
    \item Predicted Higgs mass: \(125.1\) GeV (with geometrically derived regulator)
\end{itemize}
Comparison with the observed value \(m_h = 125.11 \pm 0.14\) GeV shows good agreement within the expected theoretical uncertainty.
...
Higgs mass & \(125.1\) GeV & \(125.11 \pm 0.14\) GeV & Good \\
...
\begin{itemize}
    ...
    \item Use of the geometrically derived Dual-Zero regulator strength
    \item Small deformations of the bridge positions
\end{itemize}
Detailed convergence plots and error budgets are available in Paper 12.

\section{Conclusion}
The SAM3 framework produces realistic fermion mass hierarchies and mixing angles from purely geometric overlap integrals on the right conoid. The quantitative predictions are in good agreement with experimental data, with the Higgs mass and neutrino sum being particularly encouraging. The Dual-Zero regulator strength is determined geometrically, reducing the number of free parameters to one (\(\ell_0\)).
